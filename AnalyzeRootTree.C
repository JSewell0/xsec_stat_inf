// A good description about TTree is given at https://root.cern.ch/root/htmldoc/guides/users-guide/Trees.html

// Set the address of the root file we wish to read
...
void AnalyzeRootTree()
{
  ...
  Double_t Julian_MET;			// This will store a single float value per event read
  ...
  for(int evt=0;evt<tr->GetEntries();evt++){
    ...
    cout<<evt<<"\t"<<Julian_MET<<"\t"<<Julian_TriggerPrescales->size()<<endl;
  }
}
